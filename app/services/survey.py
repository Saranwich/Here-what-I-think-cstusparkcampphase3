...
หนึ่งใบ = หนึ่งเรื่อง:
- **ข้อนี้ใช้กับเรื่องที่โผล่มาทีหลังเท่านั้น ไม่ใช้กับข้อความเดียวที่มีหลายเรื่องพร้อมกัน**
  เช่น "แถวบ้านทั้งร้อนทั้งน้ำท่วม แล้วทางเท้าก็พัง" — นั่นคือเขาเล่ามารวดเดียว
  ให้**เก็บทุกเรื่องลง notes ให้ครบ** แล้วเลือก category จากเรื่องที่หนักที่สุด
  **ห้ามหยิบมาเรื่องเดียวแล้วทิ้งที่เหลือ** เขาพิมพ์มาแล้ว จะให้หายไปเฉย ๆ ไม่ได้
- **ถ้าเขาเล่าเรื่องใหม่ที่ไม่เกี่ยวกับเรื่องเดิม ห้ามเขียนทับเรื่องเดิม**
  เช่น กำลังคุยเรื่องน้ำท่วมอยู่ แล้วเขาพูดขึ้นมาว่าหมาจรจัดกัดคน
  นั่นคือคนละเรื่อง ไม่ใช่ข้อมูลเพิ่มเติมของเรื่องเดิม
- เรื่องแรกที่เขาเล่าคือเรื่องของใบนี้ **ทำใบนี้ให้จบด้วยเนื้อเรื่องเดิม**
  แล้วค่อยบอกเขาว่าอีกเรื่องเล่ามาได้เลย เดี๋ยวเก็บแยกให้เป็นคนละเรื่องกัน
- ที่ห้ามเพราะการเขียนทับทำให้**เรื่องที่เขาอุตส่าห์เล่าหายไปทั้งเรื่อง**
  เขาจะไม่รู้เลยว่าหาย แล้วจะไม่มีใครได้อ่านมันอีก
- **ช่องที่กรอกไว้จากเรื่องเดิม ห้ามปล่อยค้างไว้ให้เรื่องใหม่**
  เวลาที่น้ำท่วมไม่ใช่เวลาที่หมากัดคน ช่องที่ค้างมาแบบนี้ทำให้ทั้งใบผิด
...
def _status(report: dict) -> str:
    ...
    if goal is None:
        return (
            ...
            "พูดถึงปลายทางว่าเรื่องของเขาจะไปอยู่ที่ไหน "
            "**ห้ามรายงานสถานะของเอกสาร** มันฟังดูเหมือนเสมียน "
    ...
    if goal == "location":
        return "[ระบบ] ยังไม่มีตำแหน่ง — ขอตำแหน่งที่เกิดเรื่อง"

    if goal == "photo":
        return (
            "[ระบบ] ยังไม่มีรูป — ชวนถ่ายรูปตรงนั้นมาให้ดูสักรูป "
            "ขอครั้งเดียว บอกเหตุผลสั้น ๆ และทำให้ปฏิเสธง่าย"
        )

    return f"[ระบบ] ยังขาด: {FIELD_NAMES[goal]} — ถามต่อเรื่องเดียว"
