...
# หนึ่ง session เปิดได้หลายใบ แต่ละใบมีเลขของมัน — **เลขเป็นของที่เราแจก ไม่ใช่ของที่ AI ตั้ง**
# เคยให้ AI ตั้งชื่อเรื่องเองแล้วใช้ชื่อเป็นกุญแจ ปรากฏว่ามันเรียบเรียงชื่อใหม่ทุกตา
# ("ทางเท้าพัง" -> "ทางเท้าพังซอยบี") แล้วของเรื่องเดียวกันก็แตกเป็นสองใบ
# ตัวเลขที่เราพิมพ์ให้มันดูทุกตา ลอกผิดยากกว่าชื่อที่มันต้องคิดขึ้นใหม่เองมาก
MAX_OPEN_REPORTS = 4
        ...
        "parameters": {
            ...
            "properties": {
                "report_no": {
                    "type": "integer",
                    "description": (
                        "เลขของเรื่องที่ข้อมูลชุดนี้เป็นของมัน "
                        "**ระบบจะบอกเลขของทุกเรื่องที่เปิดค้างอยู่กลับมาให้ทุกตา ใส่เลขนั้นมา** "
                        "เรื่องเดิมใช้เลขเดิมเสมอ ต่อให้เพิ่งรู้รายละเอียดใหม่หรือเขาแก้ที่อยู่ก็ตาม "
                        "**ใส่เลขที่ยังไม่มีในระบบ = เปิดเรื่องใหม่** "
                        "ให้ทำแบบนั้นเฉพาะตอนที่เขาเล่าเรื่องที่คนละที่หรือคนละปัญหากับที่คุยค้างอยู่จริง ๆ "
                        "เช่น กำลังคุยเรื่องน้ำท่วมซอย 5 แล้วเขาพูดขึ้นมาว่าหมาจรจัดกัดคนที่ปากซอย "
                        "ทั้งสองเรื่องจะถูกเก็บแยกกันครบ ไม่มีเรื่องไหนถูกเขียนทับหาย "
                        "ไม่ส่งช่องนี้มา = ยังเป็นเรื่องเดิมที่คุยค้างอยู่"
                    ),
                },
                "category": {
                    "type": "string",
...
หลายเรื่องพร้อมกัน — ใช้ช่อง report_no แยก:
- **ระบบเก็บได้หลายเรื่องพร้อมกันในบทสนทนาเดียว** แต่ละเรื่องมีเลขของมัน
  ระบบจะบอกเลขที่เปิดค้างอยู่กลับมาให้ทุกตา ข้อมูลที่คุณส่งจะลงเรื่องที่ report_no ตรงกัน
- **เรื่องเดิมใช้เลขเดิมเสมอ** ต่อให้เพิ่งรู้ชื่อซอย หรือเขาแก้ที่อยู่ที่เคยบอกมา
  ก็ยังเป็นเรื่องเดิมเลขเดิม ใส่เลขใหม่เมื่อไหร่จะได้สองใบสำหรับเรื่องเดียว
  แล้วใบเก่าจะค้างอยู่พร้อมข้อมูลที่ผิดซึ่งเขาบอกให้แก้ไปแล้ว
- **ที่เดียวกันและเกี่ยวเนื่องกัน = ใบเดียว** เช่น "แถวบ้านทั้งร้อนทั้งน้ำท่วม แล้วทางเท้าก็พัง"
  นั่นคือสภาพของที่เดียวกัน ให้**เก็บทุกเรื่องลง notes ให้ครบ** แล้วเลือก category จากเรื่องที่หนักที่สุด
  **ห้ามหยิบมาเรื่องเดียวแล้วทิ้งที่เหลือ** เขาพิมพ์มาแล้ว จะให้หายไปเฉย ๆ ไม่ได้
- **คนละที่ หรือคนละปัญหาที่ไม่เกี่ยวกันเลย = คนละใบ ใส่เลขใหม่**
  เช่น กำลังคุยเรื่องน้ำท่วมซอย 5 อยู่ แล้วเขาพูดขึ้นมาว่าหมาจรจัดกัดคนที่ปากซอย
  ใส่เลขถัดไป ทั้งสองเรื่องจะอยู่ครบ **ห้ามส่งเรื่องใหม่ไปด้วยเลขเดิม**
  เพราะมันจะทับเรื่องเดิมจนหายไปทั้งเรื่อง แล้วเขาจะไม่มีวันรู้ว่าหาย
- **ช่องของเรื่องหนึ่ง ห้ามหลุดไปอยู่ในใบของอีกเรื่อง**
  เวลาที่น้ำท่วมไม่ใช่เวลาที่หมากัดคน ช่องที่ข้ามใบมาแบบนี้ทำให้ทั้งใบผิด
- **คุยทีละเรื่อง อย่าไล่ถามสองเรื่องสลับกันในตาเดียว** ระบบจะบอกเองว่าตานี้ควรถามเรื่องไหนต่อ
  รับเรื่องใหม่ไว้สั้น ๆ ว่าเก็บให้แล้ว แล้วคุยเรื่องที่ระบบบอกให้จบก่อน
...
- **แต่ถ้าเขาบอกว่าเดี๋ยวจะไปถ่ายมาให้ ห้ามใส่ no_photo เด็ดขาด**
  นั่นคือคำว่า "รอแป๊บ" ไม่ใช่คำว่า "ไม่มี" ใส่ธงไปใบจะปิดทันที
  แล้วรูปที่เขาเดินไปถ่ายจะกลับมาไม่เจอเรื่องของตัวเอง ให้ตอบรับสั้น ๆ แล้วรอเขาจริง ๆ
...
# ใบที่ขาดของบังคับ (notes/category) เร่งกว่าใบที่ขาดแค่ตำแหน่งหรือรูป
